...
(function () {
  'use strict';
  if (window.__MAS_VER__ === '1.5.20') return;
  window.__MAS_VER__ = '1.5.20';
  ...
  /** 信息流下一则：键盘 + 滚轮 + 官方切换按钮（单靠 ArrowDown 常无效） */
  function swipeToNextFeed() {
    pressArrowDown();
    try {
      const midX = Math.floor(window.innerWidth / 2);
      const midY = Math.floor(window.innerHeight / 2);
      const hit = document.elementFromPoint(midX, midY) || getVideoEl() || document.body;
      const wheel = { deltaY: 900, deltaMode: 0, bubbles: true, cancelable: true, clientX: midX, clientY: midY };
      hit.dispatchEvent(new WheelEvent('wheel', wheel));
      (hit.closest('[data-e2e="feed-active-video"], .swiper-slide-active') || hit)
        .dispatchEvent(new WheelEvent('wheel', wheel));
    } catch { /* ignore */ }
    const nextBtn = document.querySelector(
      '[data-e2e="video-switch-next-arrow"], [data-e2e="feed-scroll-down"],'
      + '.xgplayer-playswitch-next, button[aria-label*="下"], [class*="arrow-down"], [class*="ArrowDown"]',
    );
    if (nextBtn) {
  ...
  function inspectActiveFeedCard() {
    const root = getActiveFeedRoot()
      || document.elementFromPoint(window.innerWidth / 2, window.innerHeight / 2)
        ?.closest('[data-e2e="feed-active-video"], [data-e2e="feed-item"], .swiper-slide-active');
    const out = { ad: false, live: false, shop: false, text: '' };
    if (!root) return out;
    out.text = lightText(root, 360, 48);
    if (root.querySelector(
      '[class*="ad-tag"], [class*="AdTag"], [class*="advert"], [class*="Advert"],'
      + '[data-e2e*="ad"], [data-e2e*="Ad"], [class*="isAd"], [class*="is-ad"]',
    )) out.ad = true;
    if (root.querySelector(
      '[class*="live"], [class*="Live"], [data-e2e*="live"], [class*="living"]',
    ) && /直播|LIVE|进入直播/i.test(out.text)) out.live = true;

    const nodes = root.querySelectorAll('span, a, p, div, label, i');
    const max = Math.min(nodes.length, 100);
    for (let i = 0; i < max; i += 1) {
      const el = nodes[i];
      if ((el.children?.length || 0) > 3) continue;
      const t = String(el.textContent || '').replace(/\s+/g, ' ').trim();
      if (!t || t.length > 20) continue;
      if (t === '广告' || t === '广告.' || /^广告$/.test(t) || /^广告\s*$/.test(t)) out.ad = true;
      if (/^(LIVE|直播中|正在直播)$/i.test(t) || /点击进入直播间|进入直播间/.test(t)) out.live = true;
  ...
  function isFeedAdLike(text) {
    ...
    if (/\b广告\b|Advertisement|赞助商|品牌合作/i.test(text)) return true;
    if (/赞助|品牌合作|立即购买|商品橱窗/.test(text) && /广告|购物|推广/.test(text)) return true;
    return false;
  }

  function isFeedLive(text) {
    return /点击进入直播间|进入直播间|直播中|正在直播|\bLIVE\b/i.test(text || '');
  }
