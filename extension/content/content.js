/* Media Ad Skip — extension content (generated by scripts/build-extension-content.mjs) */
(function () {
  'use strict';
  ...
  const MIN_AD_SEC = 8;
  const MAX_AD_SEC = 420;
  const MIN_SEG_START_SEC = 3;
  const SEG_TAIL_GUARD_SEC = 3;
  const JUMP_VOTE_MIN_SCORE = 1.8;
  const CONF_LANG_TIP = 1.5;
  const CONF_COLON_JUMP = 1.25;
  const CONF_COLON_WEAK = 0.35;
  const MAX_FEED_BODY = 350000;
  const MAX_DANMAKU_XML = 600000;
  const MAX_RENDER_DATA = 250000;
  const MAX_PAGE_FETCH = 600000;
  const MAX_SUBTITLE_AD_SEC = 75;
  const MAX_BRAND_AD_SEC = 90;
